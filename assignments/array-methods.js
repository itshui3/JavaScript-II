...
runnersLargeSizeShirt = runners.filter( runner => { return runner.shirt_size === 'L'});
...
// Problem 1 : We want to see which companies are donating over 100
// generate an array populated with these companies' names and how much they donated
...
runners.forEach( (item) => {
  if(item.donation > 100) {
    ...
    highRollers.push({ company_name: item.company_name, donation: item.donation });
  }
});
